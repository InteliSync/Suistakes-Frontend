...
var Sound = true; //Enable sound
var isFullscreen = false;
var showTutorial = true; //Show tutorial on first play
var saveMoney = true; //Save and load player cash, set to 'false' if you don't want to save/load player money
var Cash = 500; //Dafault amount
var reloadCash = 500; //Cash reload amount if cur cash are '0'
var reloadCoin = true; //Reload coin/cash if player are lose (cur cash = 0)

//All game functions here
var Game = {
	ready: true,
	...
	loaded: function(){ //After all assets loaded
	...
	img: function(id){ //Shortcut to get image/sprite file
		return preload.getResult(id);
	},
	playSound: function(id){
		if(Sound == true){ //Shortcut to play sound
			createjs.Sound.play(id);
		}
	},
	pointCenter: function(obj){ //Set bitmap/image point to center of image
	...
	init: function(){ //Initialize
		...
		//Checking and load player money if exist
		if(saveMoney == true){
			var savedCash = localStorage.getItem('cash');
			if(savedCash !== null){
				Cash = Math.round(savedCash);
			}
		}
		...
		buttons.on('click', this.buttonClick); //Button click
	},
	gameMenu: function(){
	...
	gamePlay: function(){
		...
		//Show game tutorial on first playing
		...
		function showTutor(){
			...
			var tutor = new createjs.Bitmap(Game.img('tutor'));
			tutor.setTransform(640, 360);
			tutor.alpha = 0;
			tutor.scaleX = tutor.scaleY = 0.6;
			Game.pointCenter(tutor);

			var bClose = new createjs.Bitmap(Game.img('btnClose'));
			bClose.setTransform(1035, 134);
			bClose.alpha = 0;
			Game.pointCenter(bClose);

			stage.addChild(tutor, bClose);

			bClose.on('click', function(){
				stage.removeChild(tutor);
				stage.removeChild(bClose);
				Game.ready = true;
			});

			createjs.Tween.get(tutor).to({scaleX: 1, scaleY: 1, alpha: 1}, 500).call(function(){
				createjs.Tween.get(bClose).to({alpha: 1}, 200);
			});
		}
	},
	...
	drawHeader: function(){
		...
		buttons.addChild(btnMenu, btnSound, btnFullscreen);
